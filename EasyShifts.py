...
SHEET_CURRENT_MONTH = 'CurrentMonth'                                    # Sheet name
...
CURRENT_MONTH_UPDATE_RANGE = SHEET_CURRENT_MONTH + '!' + CURRENT_MONTH_UPDATE_CELLS    # Update range
CURRENT_MONTH_BALANCE_RANGE = SHEET_CURRENT_MONTH + '!' + CURRENT_MONTH_BALANCE_CELLS  # Balance range
...
debug = False                       # Change to True if you need some more information


# From here the magic happens.
def main():
    createOptions()                 # dayOptionsList now contains list of DayOptions Object for each day in the month
    createEasyArrangement()         # Creating a work arrangement and updating the data in the Google Sheet
...
def createOptions():
    print('*** Retrieving Options data from the Google Sheet')
...
def createEasyArrangement():
    print('*** Creating WorkDay Objects from our data')
    count = 0
    workDay = None
    for dayOption in dayOptionsList:
        if isinstance(dayOption, DayOptions):
            ...
            count += 1
    print('*** Successfully updated all shifts in the Google Sheet')


# Returns a correct cell detination for writing data into according to 'count'
# We increment the first column each time until we need to go down to another row and then we start over
...
# Writes the 'workDay' data to the current month in the desired 'cellRange' destination in the Google Sheet
def writeDataToSheet(workDay, cellRange):
    ...
    # Will be written in rows, for example 3 values will be written in cells B3, B4 and B5
    value_range_body = {
        "majorDimension": "ROWS",
        "values": writeData
    }
    request = sheetValues.update(spreadsheetId=SPREADSHEET_ID,
                                 range=SHEET_CURRENT_MONTH + "!" + cellRange,
...
def assignShiftsToWorkDay(dayOption, workDay):
    ...
    shiftIndex = 1  # Sort according to morning
    assignShiftToWorkDay(dayOption, workDay, shiftIndex, balanceList)
    shiftIndex = 2  # Sort according to Noon
    assignShiftToWorkDay(dayOption, workDay, shiftIndex, balanceList)
    shiftIndex = 3  # Sort according to Night
    assignShiftToWorkDay(dayOption, workDay, shiftIndex, balanceList)


# Prioritizes people who has less shifts so they get the shift if they can work in that date
def assignShiftToWorkDay(dayOption, workDay, shiftIndex, balanceList):
    assigned = False
    sortListsByLowestShiftIndex(balanceList, shiftIndex)
    tempBalanceList = balanceList
    if shiftIndex == 1:
        setList = list(dayOption.getMorningOptionsSet())
        while len(tempBalanceList) > 0:
            if tempBalanceList[0][0] not in setList:
                tempBalanceList.remove(tempBalanceList[0])
            else:
                workDay.setMorningWorker(tempBalanceList[0][0])
                assigned = True
                break
    elif shiftIndex == 2:
        ...
        while len(tempBalanceList) > 0:
            if tempBalanceList[0][0] not in setList:
                ...
                workDay.setNoonWorker(tempBalanceList[0][0])
                assigned = True
                break
    elif shiftIndex == 3:
        ...
        while len(tempBalanceList) > 0:
            if tempBalanceList[0][0] not in setList:
                ...
                workDay.setNightWorker(tempBalanceList[0][0])
                assigned = True
                break
    if assigned:
        dayOption.removeWorkerFromDayIfAssigned(tempBalanceList[0][0])
